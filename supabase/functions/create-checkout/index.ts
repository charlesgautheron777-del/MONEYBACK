...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const publicSiteUrl = (Deno.env.get('MONEYBACK_SITE_URL') || '').replace(/\/$/, '');
    const auditPriceId = Deno.env.get('MONEYBACK_AUDIT_PRICE_ID');
    ...
    if (!companyName || !email || !email.includes('@')) {
      throw new Error('Entreprise et email professionnel requis');
    }
    ...
    const origin = publicSiteUrl || req.headers.get('origin') || 'https://moneyback.fr';

    const lineItem = auditPriceId
      ? { price: auditPriceId, quantity: 1 }
      : {
          price_data: {
            currency: 'eur',
            product_data: {
